...
DROP FUNCTION IF EXISTS listagem_novos_bibliograficos(date, date);
...
SELECT
    i.jsonb->>'hrid',
    i.jsonb->>'title',
    to_char((i.jsonb->'metadata'->>'createdDate')::date, 'YYYY-MM-DD'),
    (i.jsonb->'metadata'->>'createdDate')::timestamptz,
    u.id,
    u.jsonb->>'username',
    concat_ws(' ',
...
WHERE (i.jsonb->'metadata'->>'createdDate')::date BETWEEN start_date AND end_date
  AND i.__current
ORDER BY
    (i.jsonb->'metadata'->>'createdDate')::timestamptz DESC,
    concat_ws(' ',
        u.jsonb->'personal'->>'firstName',
        u.jsonb->'personal'->>'lastName')
